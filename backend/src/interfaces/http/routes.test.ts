...
vi.mock('../../infra/repositories/prisma-item-repo', () => {
  class FakeRepo {
    ...
    async listChildren(parentId: string | null) { return [] }
    async create(data: any) { return { id: 'new', ...data } }
    async update(id: string, data: any) { return { id, name: data.name ?? 'n', parentId: data.parentId ?? null, type: 'folder' } }
    async delete() { return }
    async search({ q, limit }: any) { return { items: [{ id: 's1', name: q, parentId: null, type: 'file' }], nextCursor: null } }
  }
...
describe('apiV1 routes', () => {
  ...
  const res = await app.fetch(makeReq('http://localhost:3000/api/v1/items/00000000-0000-0000-0000-000000000000'))
    expect(res.status).toBe(404)
  })

  it('search validates q', async () => {
  const res = await app.fetch(makeReq('http://localhost:3000/api/v1/items/search'))
    expect(res.status).toBe(400)
  })

  it('search returns results', async () => {
  const res = await app.fetch(makeReq('http://localhost:3000/api/v1/items/search?q=test&limit=5'))
    const body = await json(res)
    expect(res.status).toBe(200)
  ...
  const res = await app.fetch(makeReq('http://localhost:3000/api/v1/items/11111111-1111-1111-1111-111111111111', { method: 'DELETE' }))
